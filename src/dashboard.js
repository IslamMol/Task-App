...
function homeSection(key,tasks,done,total){
  const wrap=(content)=>`<section data-home-section="${key}">${content}</section>`;
  if(key==='habit') return wrap(`<div class="section-head"><h2>Привычки</h2><div class="section-actions"><button class="add-circle" onclick="window.setActiveAdd('habit');openAdd()">＋</button><button class="view-all" onclick="window.setActiveAdd('habit');openAdd()">Все ›</button></div></div><div style="margin-top:10px">${habitStrip()}</div>`);
  if(key==='note') return wrap(`<div class="section-head"><h2>Задачи на сегодня</h2><div class="section-actions"><button class="add-circle" onclick="openAddForQuest()">＋</button><button class="view-all" onclick="switchMainTab('tasks')">Все ›</button></div></div><div style="margin-top:10px">${taskRows(tasks)}</div><div class="completed-card" style="margin-top:10px"><div><div class="completed-label">ВЫПОЛНЕНО СЕГОДНЯ</div><div class="completed-value">${done} из ${total}</div></div><button class="primary-pill" onclick="openAddForQuest()">＋ Задача</button></div>`);
  if(key==='finance') return wrap(`<div class="section-head"><h2>Финансы</h2><div class="section-actions"><button class="add-circle" onclick="openFinanceInfo('expense')">＋</button><button class="view-all" onclick="switchMainTab('finance')">Все ›</button></div></div><div style="margin-top:10px">${financeSnapshot()}</div>`);
  if(key==='book') return wrap(`<div class="section-head"><h2>Чтение</h2><div class="section-actions"><button class="add-circle" onclick="window.setActiveAdd('book');openAdd()">＋</button><button class="view-all" onclick="window.setActiveAdd('book');switchMainTab('tasks');renderTasksPage('book')">Все</button></div></div><div style="margin-top:10px;padding-bottom:10px">${readingSnapshot()}</div>`);
  return '';
}
